// glprim.cpp : Defines the initialization routines for the DLL.
//
...
#define setup(i,b0,b1,r0,r1)\
	t = vec[i] + N;\
	b0 = ((int)t) & (mMaxSample-1);\
	b1 = (b0+1) & (mMaxSample-1);\
	r0 = t - (int)t;\
	r1 = r0 - 1.0f;
...
Perlin::Perlin(int octaves,double freq,double amp,int seed, int iSample)
{
	mOctaves = octaves;
	mFrequency = freq;
	mAmplitude = amp;
	...
	mStart = true;
	mMaxSample = iSample;
	p=new int[mMaxSample + mMaxSample + 2];
	g1=new double[mMaxSample + mMaxSample + 2];
	g2=new double[mMaxSample + mMaxSample + 2][2];
	g3=new double[mMaxSample + mMaxSample + 2][3];
}
