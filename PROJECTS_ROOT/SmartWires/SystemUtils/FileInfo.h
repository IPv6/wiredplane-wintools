...
*  1999-9-16 ATL: Corrected bug in GetCRC and GetChecksum as suggested by Rуbert Szucs (rszucs@cygron.hu):<nl>
...
public:
	...
	enum { 
		AP_NOSORT			=0,         /** @@emem Insert <c CFileInfo>s in a unordered manner */
			...
			AP_SORTDESCENDING	=1, /** @@emem Insert <c CFileInfo>s in a descending number */
									...
									AP_SORTBYTIME		=8      /** @@emem AP_SORTBYTIME | Insert <c CFileInfo>s ordered by modification date (presumes array is 
									previously ordered by date) */
	};
    ...
    */
	...
	// Всплывание директорий в списке
	int BubbleUpDirectories();
	// Дополнительная защита, чтобы в список не попадало дубликатов
	BOOL ProtectFromDupes(BOOL bNewValue);
protected:
