...
// bro      2002-10-29
//========================================================
#define RETURN_XML(var) {iEndPos=(var-pi->startXML);return var;};
...
bool UTF2U(const std::string &src,std::wstring &out)
{
    ...
    for(const char *in = src.c_str(); in<src.c_str() + src.length(); in++)
    {
        if(0 == mState) {
            if( 0 == (0x80 & (*in))) {
                // ASCII
                ...
                if(0 == --mState) {
                    if(mUcs4 >= 0x00010000) {
                        if(mUcs4 >= 0x00110000) {
                            ...
                            mUcs4 -= 0x00010000;
                            out += (wchar_t)(0xD800 | (0x000003FF & (mUcs4 >> 10)));
                            out += (wchar_t)(0xDC00 | (0x000003FF & mUcs4));
                        }
                    } else {
...
bool U2UTF(const std::wstring &src,std::string &out)
{
    bool res = true;
    wchar_t c;
	out.erase();
	out.reserve(src.size() * 2);
	for(const wchar_t *in = src.c_str(); in < src.c_str() + src.length(); in++)
	{
		c = (*in);
		if (*in < 0x80) {
			out += (char)c;
...
CString GetInstring(const char* szStart, const char* szEnd, CString& szString, int* iFrom)
{
	...
	if(iStartPos!=-1){
		...
		if(iEndPos!=-1){
			return szString.Mid(iStartPos,iEndPos-iStartPos+strlen(szEnd));
		}
	}
	return "";
}
...
XDoc* parseXML(CString& s)
{
	char* szXML=strdup(s);
	char* szOriginals=szXML;
	CString sEnc=GetInstring("<?","?>",s,0);
	sEnc.MakeLower();
	if(sEnc.Find("utf8")!=-1 || sEnc.Find("utf-8")!=-1){
		std::string sIn(szXML);
		std::wstring sOut;
		if(UTF2U( sIn, sOut)){
			USES_CONVERSION;
			free(szOriginals);
			szXML=strdup(W2A(sOut.c_str()));
			s=szXML;
			szOriginals=szXML;
		}
	}
