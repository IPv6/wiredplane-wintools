	...
	crypt_info* cryptinf;
{
	...
	if (cryptinf!=NULL)
		memcpy(&zi->cr,cryptinf,sizeof(crypt_info));
	else
		strcpy(zi->cr.pwd,"");
...
// made for password checking
	...
	if (zi->cr.pwd[0]!=0){
		if (!zi->cr.isCalcCRC)
			// it's for winZip but for pkzip we must need no CRC
			crypthead(cryptinf->pwd,zi->ci.dosDate<<16,zi->filezip,&zi->cr); 
		else
			crypthead(cryptinf->pwd,cryptinf->CalcCRC,zi->filezip,&zi->cr); 
	}
	zi->ci.crc32=0;
    ...
    unsigned len;
{
    ...
    while ((err==ZIP_OK) && (zi->ci.stream.avail_in>0))
    {
		...
		// made for password checking
		if (zi->cr.pwd[0]!=0)
			cryptwrite(zi->ci.buffered_data,(uInt)zi->ci.pos_in_buffered_data,1,zi->filezip,&zi->cr);
		else
    ...
    zipFile file;
{
    ...
    ziplocal_putValue_inmemory(zi->ci.central_header+20,
                                (uLong)zi->ci.stream.total_out+12,4); /*compr size*/
    ...
    if (err==ZIP_OK)
    {
        ...
        if (err==ZIP_OK) /* compressed size, unknown */
            err = ziplocal_putValue(zi->filezip,(uLong)zi->ci.stream.total_out+12,4); 
	    ...
	    if (fseek(zi->filezip,
		    ...
		    err = ZIP_ERRNO;
		if (zi->cr.pwd[0]!=0 && !zi->cr.isCalcCRC){
	        ziplocal_putValue(zi->filezip,(uLong)EXTLOCSIG,4);
			ziplocal_putValue(zi->filezip,(uLong)zi->ci.crc32,4); /* crc 32, unknown */
			ziplocal_putValue(zi->filezip,(uLong)zi->ci.stream.total_out+12,4); 
			ziplocal_putValue(zi->filezip,(uLong)zi->ci.stream.total_in,4);
		};
    }
