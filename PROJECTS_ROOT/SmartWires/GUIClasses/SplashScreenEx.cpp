...
BOOL CSplashScreenEx::Create(CWnd *pWndParent,LPCTSTR szText,DWORD dwTimeout,DWORD dwStyle)
{
	...
	DWORD dwSt=WS_POPUP;
	if(dwStyle & CSS_CHILD){
		dwSt=WS_CHILD;
	}
	if (!CreateEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,"SplashScreenExClass",NULL,dwSt,0,0,0,0,pWndParent->m_hWnd,NULL))
		return FALSE;

	return TRUE;
}

BOOL CSplashScreenEx::SetPosition(int x,int y, int w, int h)
{
	m_nxPos=x;
	m_nyPos=y;
	m_nBitmapWidth=w;
...
void CSplashScreenEx::SetTextDefaultFont(CFont* myFont)
{
	LOGFONT lf;
...
void CSplashScreenEx::SetFooter(LPCTSTR szText)
{
	m_strFooter=szText;
	RedrawWindow();
}

void CSplashScreenEx::SetTitle(LPCTSTR szText)
{
	m_strTitle=szText;
	RedrawWindow();
}

void CSplashScreenEx::SetBgColor(COLORREF crTextColor)
{
	m_crBgColor=crTextColor;
	RedrawWindow();
}
...
void CSplashScreenEx::DrawWindow(CDC *pDC)
{
	...
	if(m_bitmap.GetSafeHandle()!=0){
		...
		CBrush brText;
		brText.CreateSolidBrush(m_crTextColor);
		CBrush brBg;
		brBg.CreateSolidBrush(m_crBgColor);
		pDC->FillRect(&rtText,&brBg);
		pDC->FrameRect(&rtText,&brText);
		rtText.DeflateRect(2,2,2,2);
		pDC->FrameRect(&rtText,&brText);
		rtText.DeflateRect(5,5,5,5);
	}
	...
	if(m_rcText.Height()==0){
		rtText.bottom=rtText.top+rtText.Width();// Просто чтобы непусто было
		pDC->DrawText(m_strText,-1,rtText,m_uTextFormat|DT_CALCRECT);
		m_nBitmapHeight=int(rtText.Height()+2.5*GetSystemMetrics(SM_CYMENU)+20);/*Границы*/
		m_rcText.bottom=m_rcText.top+m_nBitmapHeight;
	}else{
		if(m_strTitle!=""){
			pDC->DrawText(m_strTitle,-1,rtText,m_uTextFormat);
			rtText.top+=GetSystemMetrics(SM_CYMENU);
		}
		if(m_strFooter!=""){
			CRect rtFooter(rtText);
			rtFooter.top=rtFooter.bottom-GetSystemMetrics(SM_CYMENU);
			pDC->DrawText(m_strFooter,-1,rtFooter,m_uTextFormat);
			rtText.bottom-=GetSystemMetrics(SM_CYMENU);
		}
		pDC->DrawText(m_strText,-1,rtText,m_uTextFormat);
	}
