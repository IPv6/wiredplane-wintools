...
function getKey($psw, $salt, $regname, $email, $fio, $appName, $fulldata,&$emailsent,$nsendeml,$printdebugs)
{
	...
	if($newMsg == "ERROR"){
		// Ошибка!!!
		if($printdebugs!=0){
			print "Failed to load mail template. Check config.php";
			print "<br>".$emailfile;
		}
		die();
	}
	...
	$appGenpath=$appGenpath.$salt.$regname."&EMAIL_ADDR=".$regname;
	...
	if($nsendeml!="1"){
		if(!mail($email,$emailtitile,$newMsg,$headers)){
			$emailsent=$email.": EMAIL WERE NOT SENT TO CUSTOMER!<hr>".$emailsent;
		}else{
			$emailsent=$email.": EMAIL sent successfully!<hr>".$emailsent;
		}
	}else{
