...
public:
	...
	CString Name(){
		CString sRes;
		sRes.Format("%lu*%lu, %lubpp",m_dwWidth,m_dwHeight,m_dwBitsPerPixel);
		if(m_dwFrequency>0){
			CString sFreq;
			sFreq.Format(" @%lu",m_dwFrequency);
			sRes+=sFreq;
		}
		return sRes;
	}
...
public:
  ...
  static LONG ChangeVideoModePermanentlyEx(const CVideoMode& mode);
protected:
