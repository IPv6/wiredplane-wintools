...
typedef struct _tagParseInfo
{
	...
	LPCTSTR		startXML;			// internal - xml string
	LPXDoc		doc;
	_tagParseInfo() { 
		trim_value = false; 
		entity_value = true; 
...
typedef CArray<LPXNode,LPXNode> CXMLNodes;
// XMLNode structure
typedef struct _tagXMLNode
{
	...
	LPXNode	GetChild( LPCTSTR name, BOOL bSafe=0 ); 
	...
	int FindByTagName(LPCTSTR name, BOOL bRecursive, CXMLNodes& sRes);
	...
	_tagXMLNode() { parent = NULL; doc = NULL; type = XNODE_ELEMENT; iBeginPos=-1;iEndPos=-1;}
	~_tagXMLNode();
