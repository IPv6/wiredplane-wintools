...
const DWORD CRC32[256] = {
	...
	void CFileInfo::Create(const WIN32_FIND_DATA* pwfd, const CString strFilePath, LPARAM lParam) {
		...
		if(pwfd->ftLastWriteTime.dwLowDateTime!=0 || pwfd->ftLastWriteTime.dwHighDateTime!=0){
			m_timLastWrite = pwfd->ftLastWriteTime;
		}else{
			m_timLastWrite = pwfd->ftCreationTime;
		}
		m_lParam = lParam;
	}
	...
	BOOL CFileInfoArray::ProtectFromDupes(BOOL bNewValue=TRUE)
	{
		iDupesProtection=bNewValue;
		return bNewValue;
	}
	int CFileInfoArray::AddFileInfo(CFileInfo& finf, LPARAM lAddParam) {
		int i=GetSize();
		BOOL bCheckFromStart=0;
		...
		case AP_SORTDESCENDING | AP_SORTBYNAME:
			for (i=0;((i<GetSize()) && ((*this)[i].GetFilePath()>finf.GetFilePath()));i++);
			break;
		case AP_SORTASCENDING | AP_SORTBYTIME:
			for (i=0;((i<GetSize()) && ((*this)[i].GetLastWriteTime()<finf.GetLastWriteTime()));i++);
			break;
		case AP_SORTDESCENDING | AP_SORTBYTIME:
			for (i=0;((i<GetSize()) && ((*this)[i].GetLastWriteTime()>finf.GetLastWriteTime()));i++);
			break;
		default:
			bCheckFromStart=1;
		...
		// Duplications?
		if(iDupesProtection){
			for(int j=(bCheckFromStart?0:i);j<GetSize();j++){
				if(GetAt(j).GetFilePath()==finf.GetFilePath()){
					return -1;
				}
			}
		}
	...
	int CFileInfoArray::BubbleUpDirectories()
	{
		int iBubbled=0;
		int i=GetSize()-1, iInsertedDirPos=-1;
		while(i>0 && i>iInsertedDirPos){
			if(GetAt(i).IsDirectory()){
				CFileInfo nf(GetAt(i));
				RemoveAt(i);
				InsertAt(0,nf);
				iInsertedDirPos++;
			}else{
	...
	int CFileInfoArray::AddDir(const CString strDirName, const CString strMask, const BOOL bRecurse,
		...
		if ((pbAbort == NULL) || (!*pbAbort)) 
		{
			...
			do
			{
				...
				if (nNdx != -1)
				{
					sExt = sMask.Left(nNdx);
					sMask = sMask.Right(sMask.GetLength() - nNdx - 1);
				}
				
				strText = strDirName + sExt;
				hFind = FindFirstFile(strText, &wfd);
				if (hFind != INVALID_HANDLE_VALUE) {
					do {
