...
typedef VOID CALLBACK HTMPRINT_NEXT_PAGE(HTMPRINT hPrint, LPVOID tag, /*in*/ UINT pageNo, /*inout*/ LPUINT pPageViewportHeight, /*out*/ LPBOOL pbContinue);
...
class  PrintEx
{
  ...
  static VOID CALLBACK nextPage(HTMPRINT hPrint, LPVOID tag, UINT pageNo, LPUINT pPageViewportHeight, LPBOOL pbContinue)
  {
    PrintEx *pex = (PrintEx *)tag;
    assert(pex);
    pex->onNextPage(pageNo, *pPageViewportHeight);
  }
...
public:
  ...
  virtual void onNextPage(unsigned int pageNo, unsigned int& pageViewportHeight)
  {
     pageNo; pageViewportHeight;
  }
